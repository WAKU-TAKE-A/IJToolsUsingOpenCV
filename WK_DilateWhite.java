...
/**
 * Max filter for a binary image.
 */
public class WK_DilateWhite implements ExtendedPlugInFilter, DialogListener
{
    // constant var.
    ...
    public boolean dialogItemChanged(GenericDialog gd, AWTEvent awte)
    {
        ...
        if(tap_x <= 0) { IJ.showStatus("'0 < tap_x' is necessary."); return false; }
        if(tap_y <= 0) { IJ.showStatus("'0 < tap_y' is necessary."); return false; }
        if((tap_x % 2) ==0) { IJ.showStatus("tap_x should be odd."); return false; }
        if((tap_y % 2) ==0) { IJ.showStatus("tap_y should be odd."); return false; }

        IJ.showStatus("WK_DilateWhite");
        return true;
    }   
    ...
    public void run(ImageProcessor ip)
    {
        ...
        ret = fltrMax_Binary(src, dst, imw, imh, tap_x, tap_y);
    ...
    private int fltrMax_Binary(float[] pSrc, float[] pDst, int imw, int imh, int tapX, int tapY)
    {
        int ret = ERR_OK;
        int numpix = imw * imh;
